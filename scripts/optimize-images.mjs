...
async function optimizeRasterImage(inputPath, outputBaseName) {
  ...
  const resizedHeight = Math.max(
    1,
    Math.round((sourceHeight / sourceWidth) * resizedWidth),
  );
...
function createWatermarkSvg(width, height) {
  const escapedPrimary = WATERMARK_TEXT.replace(/&/g, "&amp;")
    .replace(/</g, "&lt;")
    .replace(/>/g, "&gt;");
  const escapedSecondary = WATERMARK_TEXT_2.replace(/&/g, "&amp;")
    .replace(/</g, "&lt;")
    .replace(/>/g, "&gt;");
...
async function run() {
  ...
  if (files.length === 0) {
    const optimizedEntries = await fs.readdir(OUTPUT_DIR, {
      withFileTypes: true,
    });
    const optimizedFiles = optimizedEntries.filter((entry) => entry.isFile());

    if (optimizedFiles.length > 0) {
      console.log(
        "Nenhuma imagem fonte encontrada em src/imgs/. Mantendo arquivos existentes em src/imgs/optimized/.",
      );
      return;
    }

    console.warn(
      "Nenhuma imagem encontrada em src/imgs/ e src/imgs/optimized/ esta vazio.",
    );
    return;
  }
  ...
  for (const fileName of files) {
    ...
    console.log(
      `${fileName} -> ${result.outputFile} | ${formatMB(result.inputBytes)} -> ${formatMB(result.outputBytes)} (${ratio}% menor)`,
    );
  }
